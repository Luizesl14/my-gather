...
[x] Especificação completa documentada
[x] Estilo visual aprovado pelo usuário
[x] Todos os frames gerados e aprovados
[x] Background conforme pedido (transparente/branco/colorido)
[x] Fundo é 100% do tipo solicitado (não branco onde deveria ser transparente)
[x] Sprite legível no tamanho real
[x] Idles mostram 4 direções diferentes
[x] Walk frames mostram movimento fluido
[x] Cores consistentes entre frames
[x] Proporções mantidas entre poses
[x] Sem artefatos ou pixelação excessiva nas bordas
[x] Arquivos salvos em local correto
[x] Registrado no catálogo do jogo (se applicável)

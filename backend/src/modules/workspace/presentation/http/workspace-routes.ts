...
export async function registerWorkspaceRoutes(
  ...
  app.get("/workspaces/:workspaceId/map", async (request, reply) => {
    ...
    return {
      ...
      displayZoom: (src as Record<string, unknown>).displayZoom as number ?? 2.5,
      avatarScale: (src as Record<string, unknown>).avatarScale as number ?? 0.5,
      avatarYOffset: (src as Record<string, unknown>).avatarYOffset as number ?? 0,
      avatarXOffset: (src as Record<string, unknown>).avatarXOffset as number ?? 0,
      spawn: (src as Record<string, unknown>).spawn ?? defaultMap.spawn,
      layers: src.layers,
  ...
  app.put("/workspaces/:workspaceId/map", async (request, reply) => {
    ...
    await repository.updateMap(params.workspaceId, {
      ...
      displayZoom: body.displayZoom,
      avatarScale: body.avatarScale,
      avatarYOffset: body.avatarYOffset,
      avatarXOffset: body.avatarXOffset,
      spawn: body.spawn,
